...
class _MPIRSGDFBuilder(Int3cBuilder):
    '''
    ...
    def mpi_outcore_auxe2(self, cderi_file, intor='int3c2e', aosym='s2', comp=None,
        ...
        log.debug("Creating auxe2 swap file with MPI-IO HDF5")


        # fswap = lib.H5TmpFile(dir=os.path.dirname(cderi_file),
        #                       prefix='outcore_auxe2_swap',
        #                       driver='mpio', comm=comm)
        fswap = h5py.File("swap.h5", 'w', driver='mpio', comm=comm)
        ...
        buflen = min(max(int(max_memory*.9e6/16/naux/(nkpts_ij+1)), 1), nao_pair // size)
        # lower triangle part
        ...
        if max_buflen > buflen:
            log.warn('load imbalance factor in outcore_auxe2 %.2f',
                     (max_buflen/buflen - 1))
        ...
        steps_split = np.array_split(np.arange(nsteps), size)
        my_steps = steps_split[rank]
        my_sh_ranges = [sh_ranges[i] for i in my_steps]
        ...
        for istep, (sh_start, sh_end, nrow) in enumerate(sh_ranges):
            ...
            if istep in my_steps:
                my_row_ranges.append((row0, row1))

        print(f"rank: {rank}, my_steps: {my_steps}")
        print(f"rank: {rank}, rows: {sum(s[2] for s in my_sh_ranges)}")

        istep = 0
        #for (sh_start, sh_end, nrow), (row0, row1) in zip(my_sh_ranges, my_row_ranges):
        comm.Barrier()
        # if rank == 0:
        #     bufR = np.empty((nkpts_ij, comp, max_buflen, naux))
        #     bufI = np.empty_like(bufR)
        ...
        for (sh_start, sh_end, nrow), (row0, row1) in zip(my_sh_ranges, my_row_ranges):
            ...
            print(f"rank: {rank}, shls_slice: {shls_slice}")
            ...
            print(f"rank {rank}, outR shape {outR.shape}")
    ...
    def _outcore_dd_block(self, h5group, intor='int3c2e', aosym='s2', comp=None,
        ...
        if j_only or nkpts == 1:
            ...
            for kpt, ki_idx, kj_idx, self_conj \
                ...
                for idx in kpt_ij_idx:
                    # collectively create all datasets
                    h5group.create_dataset(f'{dataname}R-dd/{idx}',
                                           shape=(nao, nao, naux),
                                           dtype='f8')
                    h5group.create_dataset(f'{dataname}I-dd/{idx}',
                                           shape=(nao, nao, naux),
                                           dtype='f8')

            cooked_kk_iter = list(kk_adapted_iter(cell, kpts, kk_idx, enable_t_rev_sym))
            niter = len(cooked_kk_iter)
            kk_iter_subranges = np.array_split(np.arange(niter), size)
            my_subrange = kk_iter_subranges[rank]
            my_iters = [cooked_kk_iter[i] for i in my_subrange]
            ...
            for kpt, ki_idx, kj_idx, self_conj \
                ...
                for k, idx in enumerate(kpt_ij_idx):
                    dset_r = h5group[f'{dataname}R-dd/{idx}']
                    dset_r[()] = j3cR[k]
                    dset_i = h5group[f'{dataname}I-dd/{idx}']
                    dset_i[()] = j3cI[k]
    ...
    def solve_cderi(self, cd_j2c, j3cR, j3cI):
        ...
        cderi = cderi.astype(np.complex128)
        if cderi_negative is not None:
            cderi_negative = cderi_negative.astype(np.complex128)
    ...
    def gen_uniq_kpts_groups(self, j_only, h5swap, kk_idx=None):
        ...
        if j_only or nkpts == 1:
            ...
            naux = self.auxcell.nao
            n_j2c_uniq_kpts = len(j2c_uniq_kpts)

            j2c_dset = h5swap.create_dataset('j2c', (n_j2c_uniq_kpts, naux, naux),
                                             dtype='c16')

            for k, kpt in enumerate(j2c_uniq_kpts):
                if k % size == rank:
                    j2c = self.get_2c2e(j2c_uniq_kpts[k, None, :])
                    # j2c_group[f'{k}'].write_direct(j2c[0].astype(np.complex128))
                    j2c_dset[k] = j2c[0].astype(np.complex128)
                    j2c = None
    ...
    def make_j3c(self, cderi_file, intor='int3c2e', aosym='s2', comp=None,
        ...
        if h5py.is_hdf5(cderi_file):
            raise RuntimeError(f"{cderi_file} already exists. Aborting.")
        else:
            log.debug("Creating cderi_file")
            if rank == 0:
                f = lib.H5FileWrap(cderi_file, 'w')
                f['aosym'] = aosym
                f.close()
        comm.Barrier()
        feri = h5py.File(cderi_file, 'a', driver='mpio', comm=comm)
        log.debug("assigning kpts and aosym")
        feri['kpts'] = kpts


        fswap = self.mpi_outcore_auxe2(cderi_file, intor, aosym, comp, j_only,
        ...
        buflen = min(max(int(max_memory*.3e6/16/naux/(nkpts+1)), 1), nao_pair // (2*size))
        sh_ranges = _guess_shell_ranges(cell, buflen, aosym, start=ish0, stop=ish1)
        max_memory -= buflen * naux * (nkpts+1) * 16e-6 * 2



        all_kptij_indices = set()
        ...
        if j_only or nkpts == 1:
            if kk_idx is None:
                ...
                kpt_ii_idx = np.asarray(kk_idx, dtype=np.int32)
            all_kptij_indices = all_kptij_indices.union(kpt_ii_idx)
        else:
            enable_t_rev_sym = kk_idx is None
            kpt_ij_iters = list(kk_adapted_iter(cell, kpts, kk_idx, enable_t_rev_sym))
            for _, ki_idx, kj_idx, self_conj in kpt_ij_iters:
                kpt_ij_idx = ki_idx * nkpts + kj_idx
                all_kptij_indices = all_kptij_indices.union(kpt_ij_idx)
                if not (self_conj or not enable_t_rev_sym):
                    kpt_ji_idx = kj_idx * nkpts + ki_idx
                    all_kptij_indices = all_kptij_indices.union(kpt_ji_idx)
        
        # all_kptij_indices now contains all possible values of 'idx'
        # collectively create all datasets.
        comm.Barrier()
        data_group = feri.create_group(dataname)

        cols = [sh_range[2] for sh_range in sh_ranges]
        ncols = sum(cols)

        for idx in all_kptij_indices:
            if f'{idx}' not in data_group:
                idx_group = data_group.create_group(f'{idx}')
                idx_group.create_dataset('0', (naux, ncols), dtype='c16')

            # for istep in range(len(sh_ranges)):
            #     _, _, ncol = sh_ranges[istep]
            #     idx_group = data_group[f'{idx}']
            #     idx_group.create_dataset(f'{istep}', (naux, ncol), dtype='c16')
        ...
        def make_cderi(kpt, kpt_ij_idx, j2c):
            ...
            for istep in range(len(locs)-1):
                j3c = load(locs[istep], locs[istep + 1])
            # , j3c in enumerate(lib.map_with_prefetch(load, locs[:-1], locs[1:])):
                ...
                for k, idx in enumerate(kpt_ij_idx):
                    ...
                    assert cderi_negative is None
                    if not cderi.shape[0] > 0:
                        print(idx)
                        print(f"rank: {rank}")
                        print(f"j2c: {j2c}")
                        raise ValueError("bad")
                    # print(feri[f'{dataname}/{idx}/{istep}'].dtype)
                    # print(cderi.dtype)
                    if f'{dataname}/{idx}/{0}' not in feri:
                        print(f"rank: {rank}, idx: {idx}, istep: {istep}")
                        print("idx is in all_kptij_indices: ", idx in all_kptij_indices)
                        comm.Abort()
                    if feri[f'{dataname}/{idx}/{0}'].dtype != cderi.dtype:
                        print(feri[f'{dataname}/{idx}/{0}'].dtype)
                        print(cderi.dtype)
                        print(idx)
                        raise RuntimeError("")
                    col0 = locs[istep]
                    col1 = locs[istep + 1]
                    feri[f'{dataname}/{idx}/{0}'][:, col0:col1] = cderi
                    #log.debug(f"idx: {idx}, cderi shape: {cderi.shape}")
                    if cderi_negative is not None:
                        raise ValueError("Indefinite j2c is not implemented with MPI")
                        # for low-dimension systems
                        feri[f'{dataname}-/{idx}/{istep}'] = cderi_negative
                j3cR = j3cI = j3c = cderi = None

        # The distribution over MPI ranks should happen inside this function.
...
class _MPIRSNucBuilder(_MPIRSGDFBuilder):
    ...
    def get_pp_loc_part1(self, mesh=None, with_pseudo=True):
        ...
        # k_subranges = np.array_split(np.arange(nkpts), size)
        # my_k = k_subranges[rank]
        # my_kpts = kpts[my_k]
        # nmykpts = len(my_kpts)
        ...
        displs = list(lib.prange_split(ngrids, size))
        my_p0 = displs[rank][0]
        my_p1 = displs[rank][1]
        ...
        vj_accum = np.zeros_like(vj)

        for p0, p1 in lib.prange(my_p0, my_p1, Gblksize):
        ...
        allreduce_inplace_contiguous(comm, vj_accum)
        vj += vj_accum
