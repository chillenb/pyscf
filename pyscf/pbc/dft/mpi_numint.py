...
def split_range(N):
    tot = lib.prange_split(N, size)
    return list(tot)[rank]
...
def eval_ao_kpts_mpi(cell, coords, kpts=None, deriv=0, relativity=0,
                 shls_slice=None, non0tab=None, cutoff=None, out=None,
                 verbose=None, **kwargs):
    '''
    Returns:
    ...
    kp0, kp1 = split_range(nkpts)
    my_kpts = kpts[kp0:kp1]
    local_res = cell.pbc_eval_gto(feval, coords, comp, my_kpts, shls_slice=shls_slice,
                             non0tab=non0tab, cutoff=cutoff, out=out)
    shape = local_res.shape
    shape[0] = nkpts
    global_res = numpy.zeros(shape=shape, dtype=local_res.dtype)
    global_res[kp0:kp1] = local_res
    allreduce_inplace_contiguous(comm, global_res)
    return global_res
...
def nr_rks(ni, cell, grids, xc_code, dms, spin=0, relativity=0, hermi=1,
    ...
    if xctype in ('LDA', 'GGA', 'MGGA'):
        ...
        for ao_k1, ao_k2, mask, weight, coords \
                in ni.block_loop_mpi(cell, grids, nao, ao_deriv, kpts, kpts_band,
                                 max_memory):
            my_work.append(int(coords.shape[0]))
            for i in range(nset):
        ...
        allreduce_inplace_contiguous(comm, vmat)

        for i in range(nset):
            nelec[i] = comm.allreduce(nelec[i])
            excsum[i] = comm.allreduce(excsum[i])

        work = comm.gather(my_work)
        if rank == 0:
            print(f"work partition: {work}")
...
class MPINumInt(lib.StreamObject, numint.LibXCMixin):
    ...
    def nr_uks(self, cell, grids, xc_code, dms, relativity=0, hermi=1,
               kpt=numpy.zeros(3), kpts_band=None, max_memory=2000, verbose=None):
        if kpts_band is not None:
            # To compute Vxc on kpts_band, convert the NumInt object to MPIKNumInt object.
            ni = self.view(MPIKNumInt)
            nao = dms.shape[-1]
...
_MPINumInt = MPINumInt


class MPIKNumInt(lib.StreamObject, numint.LibXCMixin):
    ...
    def nr_rks(self, cell, grids, xc_code, dms, relativity=0, hermi=1,
        ...
        if 'kpt' in kwargs:
            sys.stderr.write('WARN: MPIKNumInt.nr_rks function finds keyword '
                             'argument "kpt" and converts it to "kpts"\n')
            kpts = kwargs['kpt']
    ...
    def nr_uks(self, cell, grids, xc_code, dms, relativity=0, hermi=1,
               kpts=None, kpts_band=None, max_memory=2000, verbose=None, **kwargs):
        if 'kpt' in kwargs:
            sys.stderr.write('WARN: MPIKNumInt.nr_uks function finds keyword '
                             'argument "kpt" and converts it to "kpts"\n')
            kpts = kwargs['kpt']
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
    ...
    def block_loop_mpi(self, cell, grids, nao=None, deriv=0, kpts=None,
                   kpts_band=None, max_memory=2000, non0tab=None, blksize=None):
        '''Define this macro to loop over grids by blocks.
        '''
        if grids.coords is None:
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
        if blksize is None:
            blksize = int(max_memory*1e6/(comp*2*len(kpts_all)*nao*16*BLKSIZE))*BLKSIZE
            blksize = (min(blksize, (ngrids+BLKSIZE*size-BLKSIZE)//size)//BLKSIZE) * BLKSIZE
            blksize = max(BLKSIZE, min(blksize, ngrids, BLKSIZE*2400))
        ...
        B0, B1 = split_range( (ngrids + blksize - 1) // blksize )
        P0 = B0 * blksize
        P1 = min(ngrids, B1 * blksize)

        for ip0 in range(P0, P1, blksize):
            ip1 = min(ngrids, ip0+blksize)
            coords = grids_coords[ip0:ip1]
            weight = grids_weights[ip0:ip1]
            non0 = non0tab[ip0//BLKSIZE:]
